...
class SimulationRouter:
    ...
    def drive_route(self, current_speed):
        ...
        if not self.segments or len(self.segments) == 0 or self.cur_segment_index > len(self.segments) - 1:
            self.current_state.is_finished = True
            return self.current_state.to_dict()

        seg_dist = self.segments[self.cur_segment_index]['distance']
        slope_percent = self.route[self.cur_segment_index + 1].get('slope', 0.0)

        remaining_dist = seg_dist - self.dist_on_segment
        ...
        self.dist_on_segment += moved_dist

        p0 = self.route[self.cur_segment_index]
        p1 = self.route[self.cur_segment_index +1 ]
        self.current_state.point.latitude = p0['latitude'] + (p1['latitude'] - p0['latitude']) * (1 - (remaining_dist - moved_dist) / seg_dist)
        self.current_state.point.longitude = p0['longitude'] + (p1['longitude'] - p0['longitude']) * (1 - (remaining_dist - moved_dist) / seg_dist)
        self.current_state.point.altitude = p0.get('altitude', 0.0) + (p1.get('altitude', 0.0) - p0.get('altitude', 0.0)) * (1 - (remaining_dist - moved_dist) / seg_dist)
        self.current_state.point.slope = slope_percent

        self.current_state.speed = params['speed']
        self.current_state.fuel_used += params['fuel_used']
        self.current_state.dist_traveled += moved_dist
